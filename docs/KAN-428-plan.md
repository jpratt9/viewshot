...
Both were left out of this change and filed instead, the way KAN-428 itself came out of KAN-421's open question 1. Neither is reproduced, and folding either in would have widened a ticket whose repro is the two-documents one.

1. **Should `ensureOffscreen()` invalidate a stale `rec` when it creates a document, rather than leaving it to the next read?** Left as planned. The key is forgotten at the first read after the new document answers — inside the same clipboard copy, because `closeOffscreen()` reads `rec` (`background.js:502`). A read that lands between `createDocument` and the first `pong` still sees a document and can't get an id, so it leaves the key alone for that window, on the same "can't tell" rule as a `hasDocument()` that rejects. Filed as **KAN-435**.
2. **A recorder that dies inside a document that lives on.** Identity proves the document, not the recorder: a document that is still the right one but whose `MediaRecorder` has errored would still vouch for the recording, and nothing listens for that error — `offscreen.js` sets `ondataavailable` and `onstop` but never `onerror`, and `onRecError` is wired only to the start. Filed as **KAN-434**.
