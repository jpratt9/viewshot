...
function load({ body, inner, iw = 1512, ih = 767, dpr = 2 }) {
  ...
  const context = {
    ...
    chrome: {
      scripting: { insertCSS: async () => {}, removeCSS: async () => {}, executeScript: async ({ func, args }) => [{ result: await func.apply(null, args || []) }] },
      ...
      runtime: { onMessage: { addListener() {} }, onStartup: { addListener() {} }, onInstalled: { addListener() {} }, sendMessage: async () => {} }, commands: { onCommand: { addListener() {} } }, downloads: { onDeterminingFilename: { addListener() {} } }, action: { setBadgeText: async () => {}, setBadgeBackgroundColor: async () => {} },
